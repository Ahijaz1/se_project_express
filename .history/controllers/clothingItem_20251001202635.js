...
const likeItem = async (req, res) => {
  const { itemId } = req.params;
  // accept user id from auth middleware, header, body, or query (tests may provide it)
  const userId = req.user?._id || req.header("x-user-id") || req.body.userId || req.query.userId;
  ...
  try {
    ...
    if (!item.likes.map((id) => id.toString()).includes(userId.toString())) {
