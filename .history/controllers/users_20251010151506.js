...
const createUser = async (req, res, next) => {
  try {
    ...
    // Duplicate email
    if (err.code === 11000) {
      return next(new BadRequestError("Email already exists", 409));
    }
...
const login = async (req, res, next) => {
  try {
    ...
    if (!email || !password)
      throw new BadRequestError("Email and password are required", 400);
    ...
    return next(new BadRequestError("Incorrect email or password", 401));
  }
};
